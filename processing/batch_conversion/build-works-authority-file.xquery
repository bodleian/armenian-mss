...
declare function local:normalize4Crossrefing($title as xs:string) as xs:string
{
    ...
    let $normalized1 := replace(normalize-unicode($title, 'NFKD'), '(^|\s)(the|a|an|part of|parts of|fragment of|fragments of|another)\s', '', 'i')
    let $normalized2 := 
        translate(
            translate(
                replace(
            ...
            <listBibl>
{
    ...
    (: First, extract all titles from identifiable works in the TEI files and build in-memory XML structure, 
    ...
    let $allworks as element()* := (
        ...
        for $msitem in $collection//tei:msItem[@xml:id]
            ...
            if (count($titles) eq 0 and not($msitem/tei:author) and string-length($msitem/@class) gt 0) then
                ...
                (: In Armenian there are occassionally multiple titles in individual works :)
                <work>
                    {
                    for $title in $titles
                        return (
        ...
        for $t at $pos in distinct-values($allworks//title)
            ...
            if (count($variants) gt 1) then
                ...
                <bibl xml:id="{ concat('work_', $pos) }">
                    <title type="uniform">{ $t }</title>
                    {
                    for $a in distinct-values($allworks[title = $t]/alt)
                        order by $a
                        return
                        <title type="variant">{ $a }</title>
                    }
