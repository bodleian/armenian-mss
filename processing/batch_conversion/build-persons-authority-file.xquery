...
{
    (: This does not change any non-ascii characters, just normalizes temporarily to find the same thing transliterated slightly
       differently into two different records. Also tries to match despite punctuation differences, etc. :)
        ...
        translate(
            translate(
                replace(
                    ...
                    'œ', 'oe'),
                'ṙ', 'r'),
            'áàāčéèēǰłōṙṛšŠủž', 'aaaceeejlorrssuz'),
        'ʼ', '')
            ...
            <listPerson>
{
    ...
    let $allpeople as element()* := (
        ...
        for $p in $collection//tei:msDesc//(tei:author|tei:editor|tei:persName)[not(parent::author or parent::editor or ancestor::tei:revisionDesc)]
            (: In Armenian, sometimes author and editor have child persNames and sometimes not :)
            let $names as xs:string* := 
                if ($p/child::persName) then
                    ($p/child::persName/normalize-space(string-join(.//text(), ' ')))[string-length(.) gt 0]
                else
                    ($p/normalize-space(string-join(.//text(), ' ')))[string-length(.) gt 0]
            return
            <person>
                {
                for $name in $names
